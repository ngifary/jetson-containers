...
show_help() {
    ...
    echo " "
    echo "   --with-ml       Builds additional container with ML support."
...
source scripts/docker_base2.sh
...
# check for local version of ML base container
BASE_IMAGE_ML="l4t-ml:r$L4T_VERSION-py3"

if [[ "$(sudo docker images -q $BASE_IMAGE_ML 2> /dev/null)" == "" ]]; then
	BASE_IMAGE_ML="nvcr.io/nvidia/$BASE_IMAGE_ML"
fi
...
for DISTRO in ${BUILD_DISTRO[@]}; do
	for PACKAGE in ${BUILD_PACKAGES[@]}; do
		...
		if [[ "$WITH_ML" == "on" && "$DISTRO" != "melodic" && "$DISTRO" != "eloquent" ]]; then
			build_ros $DISTRO $PACKAGE $BASE_IMAGE_ML "ml-"
		fi
	done
done
